...
$newKey = generateRandomString();
 ...
 <Item class="ModuleScript" referent="RBX4D11BCF7D08A4AC1A86EF6D9DA136530">
  <Properties>
   ...
   <ProtectedString name="Source"><![CDATA[local fat=script;script=nil;return function(key) if key == "' . $newKey . '" then return fat.Local:Clone() else return "Invalid key!" end end]]></ProtectedString>
  </Properties>
 ...
 if (stristr($response, 'Object moved')){
  ...
  $response = curl("http://data.roblox.com/Data/Upload.ashx?assetid=0&type=Model&name=NewLocalScript&description=None&genreTypeId=1&ispublic=False&allowcomments=False",$PostData,true);
 }
 $response = curl("http://www.roblox.com/ide/toolbox/items?category=MyModels&creatorId=337580063&keyword=&num=1&page=1&sort=Relevance",false,true);
 $data = json_decode($response,true);
 echo $data["Results"][0]["Asset"]["Id"],':',$newKey;
?>